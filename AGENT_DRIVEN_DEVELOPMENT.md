...
**Edit discipline.**
- Only `depends_on` is hand-edited. `derived:` is regenerated by `dag.sh --write` (a plain run is read-only and only warns when it is stale) — never hand-edit it.
...
phase_features:
  modules:
    ...
    - id: graph        # service: cross-tier graph/diff (Tier 2 per RFC-000)
      phase: 2
      depends_on: [client, storage]
```
...
| 8 | Per phase-1 feature: derive contract YAML, then run §4.1 | **to add** (start from RFC-000 work items) |
...
| **Pipeline silently persists a bad result** | `validate_tree()` before `save_doc` + a `LOW_QUALITY_TREES` metric (see RFC-000 §P0b) |
...
## Unresolved
- [GAP] 2026-05-30 | .pdf routes through PyPDF2+LLM-TOC; should go PDF→markdown→md_to_tree (RFC-000 RC1/RC3/RC4)
