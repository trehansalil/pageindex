...
| Artifact               | Reference                                                                                                                                  |
| ---------------------- | ------------------------------------------------------------------------------------------------------------------------------------------ |
| Governing RFC(s)       | [RFC-026: Run 9 — Verdict Gate Hardening &amp; Page Rotation Detection](../rfcs/026-verdict-gate-hardening-rotation-detection.md) |
...
| Priority Order         | [RFC-026 Priority Order](../rfcs/026-verdict-gate-hardening-rotation-detection.md#priority-order)                                  |
| Out of Scope           | [RFC-026 Out of Scope](../rfcs/026-verdict-gate-hardening-rotation-detection.md#out-of-scope)                                      |
...
This plan implements five of the six RFC-026 fix dimensions across three batches — [D6](../rfcs/026-verdict-gate-hardening-rotation-detection.md#out-of-scope) (Latin-gibberish CMap detection) is explicitly **deferred** and excluded from this plan per the RFC's own scope note. Batch 0 lands the three verdict-gate fixes that share `helpers.py` — [D0](../rfcs/026-verdict-gate-hardening-rotation-detection.md#d0--hard-fail-floor-for-zero-content-documents) (zero-content FAIL floor), [D1](../rfcs/026-verdict-gate-hardening-rotation-detection.md#d1--image-enrichment-promoted-volume-floor) (image-enrichment volume floor), and [D5](../rfcs/026-verdict-gate-hardening-rotation-detection.md#d5--validate-tree-garble-check-ordering) (garble-check reordering in `validate_tree()`) — batching them avoids merge conflicts and matches the RFC's own priority ordering (D0, D1, D5 are the three highest-priority, lowest-complexity items). Batch 1 lands [D2](../rfcs/026-verdict-gate-hardening-rotation-detection.md#d2--page-level-rotation-detection) (page-level rotation detection) in `converters.py`, independent of the verdict-gate logic and the largest single change in the RFC. Batch 2 lands [D3](../rfcs/026-verdict-gate-hardening-rotation-detection.md#d3--hysteresis-preservation-across-reingestion-wipe) (hysteresis snapshot across reingestion wipe, `storage.py`) and [D4](../rfcs/026-verdict-gate-hardening-rotation-detection.md#d4--scoring-harness-stage-2-guard-fix) (scoring-harness Stage 2 guard fix, `.claude/workflows/corpus-ingest-score.js`) — these are independent of each other and of Batches 0-1, and are grouped together as the two remaining lower-priority items. Per explicit user directive, **no task in this plan performs ingestion, reingestion, or corpus verification** — those steps belong to the corpus quality cycle, which runs separately after this plan lands. Every task ends at a `uv run pytest` (or, for D4, a harness-level JS assertion) unit/integration boundary.

## Tasks

- [ ] <a id="1-batch-0--verdict-gate-fixes-d0-d1-d5"></a>1. Batch 0 — Verdict Gate Fixes ([RFC-026 D0](../rfcs/026-verdict-gate-hardening-rotation-detection.md#d0--hard-fail-floor-for-zero-content-documents), [D1](../rfcs/026-verdict-gate-hardening-rotation-detection.md#d1--image-enrichment-promoted-volume-floor), [D5](../rfcs/026-verdict-gate-hardening-rotation-detection.md#d5--validate-tree-garble-check-ordering))

  *All three fixes touch `src/pageindex_mcp/helpers.py` and are independent of one another — batched together to avoid merge conflicts and because they are the RFC's top three priority items. See [Priority Order](../rfcs/026-verdict-gate-hardening-rotation-detection.md#priority-order) items 1-3.*
...
- [X] <a id="2-batch-1--rotation-detection-d2"></a>2. Batch 1 — Page Rotation Detection ([RFC-026 D2](../rfcs/026-verdict-gate-hardening-rotation-detection.md#d2--page-level-rotation-detection))

  *Independent of Batch 0's verdict-gate logic — this batch is entirely within `src/pageindex_mcp/converters.py` and addresses the persistent portrait/landscape stall documented across 3 consecutive runs. See [Priority Order](../rfcs/026-verdict-gate-hardening-rotation-detection.md#priority-order) item 4.*
  ...
  - [X] <a id="23-unit-tests-batch-1"></a>2.3 Unit tests for Tasks 2.1-2.2 (D2)
    ...
    - **Validates: [Design Property 3](../designs/design-rfc026-verdict-gate-hardening-rotation-detection.md#property-3-rotation-aware-extraction), [RFC-026 D2](../rfcs/026-verdict-gate-hardening-rotation-detection.md#d2--page-level-rotation-detection)**
  ...
  - [X] <a id="24-checkpoint--batch-1"></a>2.4 Checkpoint — Batch 1: Page Rotation Detection
    ...
    - Ask the user if questions arise before proceeding
- [X] <a id="3-batch-2--hysteresis-and-harness-d3-d4"></a>3. Batch 2 — Hysteresis & Harness ([RFC-026 D3](../rfcs/026-verdict-gate-hardening-rotation-detection.md#d3--hysteresis-preservation-across-reingestion-wipe), [D4](../rfcs/026-verdict-gate-hardening-rotation-detection.md#d4--scoring-harness-stage-2-guard-fix))

  *D3 (`storage.py`) and D4 (`.claude/workflows/corpus-ingest-score.js`) are independent of each other and of Batches 0-1 — grouped together as the RFC's two lowest-priority remaining items. See [Priority Order](../rfcs/026-verdict-gate-hardening-rotation-detection.md#priority-order) items 5-6.*
  ...
  - [X] <a id="33-unit-tests-batch-2"></a>3.3 Unit tests for Tasks 3.1-3.2 (D3, D4)
    ...
    - **Validates: [Design Property 4](../designs/design-rfc026-verdict-gate-hardening-rotation-detection.md#property-4-hysteresis-preservation), [Design Property 5](../designs/design-rfc026-verdict-gate-hardening-rotation-detection.md#property-5-scoring-harness-structured-output), [RFC-026 D3](../rfcs/026-verdict-gate-hardening-rotation-detection.md#d3--hysteresis-preservation-across-reingestion-wipe), [RFC-026 D4](../rfcs/026-verdict-gate-hardening-rotation-detection.md#d4--scoring-harness-stage-2-guard-fix)**
...
- [X] <a id="4-final-checkpoint"></a>4. Final Checkpoint
  ...
  - Confirm [D6](../rfcs/026-verdict-gate-hardening-rotation-detection.md#out-of-scope) (Latin-gibberish CMap detection) remains untouched and is not silently implemented as a side effect of any Batch 0-2 task
...
- [D6](../rfcs/026-verdict-gate-hardening-rotation-detection.md#out-of-scope) (Latin-gibberish CMap detection for Haftpflicht-class docs) is **explicitly deferred** per the RFC's own [Priority Order](../rfcs/026-verdict-gate-hardening-rotation-detection.md#priority-order) item 7 ("deferred if scope tight") and is excluded from this plan entirely — no task, test, or checkpoint in this document references it as in-scope work.
...
- [Task 1.1](#11-zero-content-fail-floor)'s [D0](../rfcs/026-verdict-gate-hardening-rotation-detection.md#d0--hard-fail-floor-for-zero-content-documents) FAIL floor must run before [Task 1.2](#12-image-enrichment-volume-floor)'s [D1](../rfcs/026-verdict-gate-hardening-rotation-detection.md#d1--image-enrichment-promoted-volume-floor) promotion-floor check in the final `classify_verdict()` control flow — the RFC's ordering requirement (D0 before `image_enrichment_promoted`, line 1198) is load-bearing: it prevents the D1 char-floor branch from ever seeing a `total_chars == 0` document, which must always resolve to `zero_content`, not `image_enrichment_promoted_below_char_floor`.
