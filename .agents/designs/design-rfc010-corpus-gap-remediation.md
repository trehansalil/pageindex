...
| Garbled flat markdown detected | Reason overridden to `"garbling"`, OCR escalation fires and typically succeeds (see `client.py` `_flat_text_is_garbled` wiring); only if OCR *also* fails does it fall through to FLAT-03 | Fall-through is a last resort, not the default outcome | [D3B](../rfcs/010-corpus-gap-remediation.md#d3--extended-garble-detection-gap-2--43-lines) | [P4](#property-4-flat-path-garble-gate) |
