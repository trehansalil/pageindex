...
quarantine/<sha256>.meta.json          filenames that triggered quarantine     [current]
...
- **Decision.** Keep PyMuPDF in both sidecars and accept the exposure explicitly, on these terms:
  ...
  deployed product surface. Term (4) turns on reachability, so it is stated per service rather than in
  aggregate. `paddleocr-vl-service` is gated behind `profiles: ["ocr-spike"]` and starts only when that
  profile is selected. **`surya-ocr-service` has no `profiles:` entry at all** — the gate was removed
  deliberately in RFC-048 (`audit/CORPUS_REINGESTION_AUDIT_RUN-21_RFC048.md:31`) — so *both* a bare
  `docker compose up` and `docker compose --profile app up` start it, while the application itself
...
- **Consequences.** Two further components fall under the unresolved AGPL §13 obligation tracked as R10.
  ...
  `surya-ocr-service` would make term (1) symmetric and shrink the accepted surface further. It is
  deliberately **not** done here: RFC-048 removed that gate on purpose and the re-ingestion workflow
  depends on the service being up, so re-adding it changes how ingests are started and is a call for the
  maintainer, not a review fix. The loopback narrowing above was taken because it has no workflow impact
  at all. Each sidecar's README carries this note so the
