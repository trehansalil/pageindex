...
- [x] 6. Content-Derived OCR Language Selection (D4)
  ...
  - [x] 6.3 Tests for D4
    ...
    - **وارد رقم 597 status update (Amendment 3, 2026-09-21):** Post-D6+D7, verdict regressed FAIL→REJECTED. Root cause: D6 task 4.3 made flat garble gate see image OCR text (`block_text` under `GARBLE_CHECK`), catching `sparse_mojibake` on Latin noise from English-only tessdata. The regression is correct behavior (more accurate garble detection). Primary defect is `rtl_reversal` (bidi_degraded_gate fires, `reversed_signal=true`), RTL recovery doesn't converge, flat route garble-rejected. D4 content-derived langs should resolve both the garble root cause (wrong tessdata) and the RTL reversal (Arabic tessdata produces correct bidi).
    ...
    - **اتفاقية status update (Amendment 3, 2026-09-21):** **Already PASS post-D7.** The D7 landscape reroute guard (`skip_tree_already_passed`) prevented the flat-force downgrade. OCR retry resolved garble (12039→34569 chars), rebuilt tree passed all gates (108 nodes, depth 4, max_leaf_ratio=0.048, chars_per_page=1728). D4 is no longer needed for this doc's verdict — it's already at PASS. D4 may still improve initial extraction quality (avoid garble→retry→rebuild cycle), but the verdict is stable.
    - **MOU MOHRE status update (Amendment 3, 2026-09-21):** Post-D7, still FAIL (`suspect_density`). D7 `pre_rebuild_md_quality` fires correctly (`md_clean`). Garble resolved by OCR retry. Blocker is density gate: `chars_per_page=1437.7 < 1500 floor`. D8 corrected numerator confirmed to clear the gate (`chars_per_page_corrected=1586.6`, `verdict_would_change=true` in 4.C). D4 may improve initial extraction (correct tessdata from the start), but density gate is the binding constraint.
