...
- [ ] 8. RFC-047 Decision Point
  ...
  - [ ] 8.2 Decide whether RFC-047 is warranted
    ...
    - **Eval-informed framing (Amendment 1, 2026-09-21):** Wave 2 data shows PaddleOCR-VL silently fails on 24% of the corpus (6/25 zero-output docs) and PaddleOCR is 10× slower than Surya on Arabic. A 4-engine strategy is not viable without addressing VL reliability. The realistic RFC-047 scope is a **2-engine strategy (Tesseract + Surya)** — Surya's zero-failure, high-confidence profile across all doc types makes it the strongest secondary. Frame the decision as: does Surya add enough over Tesseract-only (after D4/D7 fixes) to justify the operational cost?
