...
**B6 · The Run-8 tally must be corrected** — 13/6/6, not 14/6/5. **[R2]** *(Verified 2026-09-15, task 1.8: the count is confirmed; the stated cause is not. No row is omitted — all 25 are present. The divergence originates in the "Updated tally" ledger, whose post-RFC-045 row records 9/9/6/1 where the re-test it summarises yields 9/9/7/0, and whose final row then moves Doc 6 from ERROR to FAIL a second time. Recorded as a dated addendum in the audit file.)* Note `audit/CORPUS_REINGESTION_AUDIT_RUN-8.md:5` also records `Branch: ICR-97-rfc44-recovery-dispatch-wiring` — defensible, as that name and `ICR-97-rfc45-arabic-ocr-garble-fix` shared tip `3c7eda1` during the run; annotated rather than changed.
