...
| D12's emitters cost hot-path wall-clock | Every emitter is guarded by `isEnabledFor` before its `attrs` dict is built — the dict is the cost. Per-node records are DEBUG-only and capped by `garble.py`'s `_GARBLE_PRONG_EVAL_CAP` / `_GARBLE_VERDICT_CAP` (both hardcoded at 50); `PAGEINDEX_LOG_DECISIONS=off` bisects a regression without a revert. **(Corrected 2026-09-24, RFC-049:** this row named `PAGEINDEX_LOG_NODE_SAMPLE` as the cap. Nothing ever read that variable, so it capped nothing; it has been removed.**)** |
