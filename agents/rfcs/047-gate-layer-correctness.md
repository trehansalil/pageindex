...
**Exit gate:** All existing garble tests pass. New tests for parenthesized Arabic markers and digit-only bridging pass. The ratio threshold is configurable via `GarbleConfig`.
...
| D2's ratio threshold lets some garbled enrichments through | Medium | Garbled blocks below the threshold are persisted | The threshold (default 0.3) means fewer than 30% of blocks garbled is tolerated; the main flat-blocks garble gate provides a second check |
