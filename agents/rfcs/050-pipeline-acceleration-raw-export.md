...
### Requirement 7: Selective Table Structure Processing (Added: 2026-09-25)

**User Story:** As a platform operator, I want the pipeline to skip the TableFormer ACCURATE model on pages that have no tables, so that per-file extraction time is reduced without sacrificing table reconstruction quality on pages that do have tables.

**Finding:** `_build_pdf_pipeline_options()` (`converters/docling_conv.py:46-118`) unconditionally sets `opts.do_table_structure = True` and `opts.table_structure_options.mode = TableFormerMode.ACCURATE`. Every page of every text-based PDF goes through Docling's layout model (RT-DETRv2) plus TableFormer ACCURATE, even when no table is present. The German insurance T&C corpus is predominantly body text with tables on a minority of pages. A pre-extraction ruled-table heuristic using PyMuPDF's `get_cdrawings()` can identify table-bearing pages in milliseconds, letting the pipeline build a no-table-structure converter for table-free documents and chunks.

**Scope:** Text-based PDFs only. Scanned pages never reach this pipeline path (they go through OCR recovery, not the primary Docling extraction). **(Amendment 2026-09-25, Iter 11 — cascade enhancement):** The heuristic now uses a two-stage native cascade: (1) vector-geometry detection (ruled tables via `get_cdrawings()` horizontal/vertical lines and rectangles), and (2) text-block column-alignment detection (borderless tables via `page.get_text("blocks")` x-coordinate clustering). A page is positive if either signal fires. Neighbor-page padding (±1 page around every positive) provides safety margin for cross-page tables and near-miss heuristic failures. A three-tier confidence model (confident-no / ambiguous / confident-yes) replaces the prior binary decision; ambiguous pages are treated as positive (safe default — no quality regression). A future visual-detector stage (RT-DETR/ONNX on page thumbnails for ambiguous pages) is deferred to a separate deliverable (D9/R8).

#### Acceptance Criteria

1. BEFORE Docling extraction, THE pipeline SHALL scan each page of a text-based PDF using a two-stage native cascade: (a) vector-geometry heuristic via PyMuPDF's `get_cdrawings()` — horizontal lines, vertical lines, and rectangles; (b) text-block column-alignment heuristic via `page.get_text("blocks")` — counting x-coordinate positions with ≥3 text blocks at the same (quantized) x-offset; ≥2 such columns indicates a probable borderless table. A page is positive if **either** signal fires. The result is a set `pages_with_tables: set[int]` (0-indexed).
2. THE pipeline SHALL apply neighbor-page padding: for every page in `pages_with_tables`, pages at index N-1 and N+1 (clamped to `[0, page_count-1]`) SHALL also be included. This provides safety margin for tables spanning page breaks and near-miss heuristic failures.
3. THE pipeline SHALL use a three-tier confidence model for each page: (a) **confident-no** — neither vector-geometry nor column-alignment signal fires, and no neighbor is positive → page excluded from `pages_with_tables`; (b) **ambiguous** — the column-alignment signal fires weakly (exactly 2 aligned columns with borderline row count) → treated as positive (included in `pages_with_tables`); (c) **confident-yes** — vector-geometry signal fires, or strong column-alignment signal → included in `pages_with_tables`. Ambiguous pages default to positive — a false positive costs a little compute; a false negative silently loses structured information.
4. IF `pages_with_tables` is empty for the entire document (non-chunked path) or for a given chunk (chunked path), THE pipeline SHALL pass `do_table_structure=False` to `_build_pdf_pipeline_options()`, skipping TableFormer entirely for that converter invocation.
5. IF `pages_with_tables` is non-empty, THE pipeline SHALL use the existing `do_table_structure=True` / `TableFormerMode.ACCURATE` setting. There is no per-page selective processing within a single Docling invocation — the granularity is per-converter-call (whole document or chunk).
6. THE detection heuristic SHALL require `allow_agpl_fallback=True` (PyMuPDF is AGPL-3.0). When `allow_agpl_fallback=False`, the pipeline SHALL fall back to `do_table_structure=True` for all pages (no optimization, same as today).
7. THE detection heuristic SHALL be gated by an env var `TABLEFORMER_SKIP_ENABLED` (default `"1"`/true). Setting it to `"0"`/false disables the optimization entirely (all pages get TableFormer), providing a kill switch.
8. THE detection result (`pages_with_tables`) SHALL be included in the `PreClassification` dataclass and threaded through the handshake JSON, so it is available to the converter without re-scanning.
9. A detection failure (exception in `get_cdrawings()`, `get_text("blocks")`, or the heuristic) SHALL fall back to `do_table_structure=True` and log a warning — detection must never prevent extraction.
10. THE detection result SHALL include a `detection_method: str` field on `PreClassification` recording which signal(s) fired (`"vector"`, `"column_alignment"`, `"vector+column_alignment"`, `"neighbor_pad"`, or `None` when detection unavailable), for observability and future tuning.
...
**D8 (Requirement 7) — P1 — New, 2026-09-25; amended 2026-09-25 Iter 11 (cascade enhancement):** Two-stage native cascade table detection in `detect_pages_with_tables(pdf_path: str) -> set[int] | None` (`converters/preclassify.py`), called from `preclassify_document()`. The function opens the PDF with `fitz`, iterates pages, and applies two complementary heuristics per page: (a) **vector-geometry** — count horizontal lines (slope ≈ 0), vertical lines (slope ≈ ∞), and rectangles from `page.get_cdrawings()`; a page has a ruled table when it has ≥3 horizontal and ≥3 vertical elements; (b) **text-block column alignment** — extract text blocks via `page.get_text("blocks")`, quantize x-coordinates, count positions with ≥3 blocks; ≥2 such columns indicates a probable borderless table. A page is positive if **either** signal fires. **Neighbor-page padding** (±1 page around every positive, clamped to valid range) provides safety margin for cross-page tables. A **three-tier confidence model** classifies pages as confident-no / ambiguous / confident-yes; ambiguous pages (weak column-alignment signal) are treated as positive — a false positive costs compute, a false negative silently loses data. Returns `None` when detection is unavailable (AGPL gate off, `fitz` import failure, or `TABLEFORMER_SKIP_ENABLED=0`), signaling "assume tables everywhere." `PreClassification` gains `detection_method: str | None` for observability (`"vector"`, `"column_alignment"`, `"vector+column_alignment"`, `"neighbor_pad"`, `None`). The result flows through: (1) `pages_with_tables: set[int] | None` field on `PreClassification`, serialized as `list[int]`; (2) the existing handshake JSON — no new protocol field; (3) `pdf_to_markdown_docling()` accepts only `pages_with_tables: set[int] | None` — `do_table_structure` is computed internally, not exposed as a public parameter; (4–6) internal converter layers thread `do_table_structure` through `_build_pdf_pipeline_options()`, `_docling_converter()` cache key, and the chunked path's per-chunk intersection; (7) the remote/service path: `PdfConvertRequest` carries `pages_with_tables: list[int] | None`, converted to `set` at the boundary. Detection cost is sub-millisecond per page for vector-geometry, low-millisecond for text-block analysis. **Future (D9, not this RFC):** visual detector stage (RT-DETR/ONNX on page thumbnails) for ambiguous pages where neither native signal is conclusive.
...
| 2b | D8 (selective TableFormer) — table detection gate in preclassify, `do_table_structure` parameter threading through local + remote paths, tests | ~4h | Low |
...
**Total: ~20h.** D4 (replay CLI) and D5 (recovery optimization) are deferred (see their Decision Summary entries) and excluded from this total; their prior Iter 8 estimate (17.5-19.5h + 4-6h) is the bulk of what Iter 9's MINIMAL-scope cut removes from the 37-41h Iter 8 total.
