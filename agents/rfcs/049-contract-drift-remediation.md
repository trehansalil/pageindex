...
> **Partially superseded by [[RFC-050]] D6 (2026-09-24): the 30-day quarantine lifecycle TTL is withdrawn.** Every clause in this RFC that says `quarantine/` objects expire after 30 days through a MinIO lifecycle rule (Glossary "Quarantine", D2-C retention, R4 AC5, Risk 6, Open Question 4, the decision tables) no longer applies. The TTL was never in force: Task 7.5c is recorded STILL OPEN, and `get_bucket_lifecycle()` returned `None` on the local bucket (2026-09-23). Quarantine is now bounded only by clear-on-success (Task 7.5a) and on-demand erasure (`delete_doc` via `ctx.sha256`; `erase_quarantine(sha256)`, sha256 surfaced per Task 7.5d). Everything else in RFC-049 stands. RFC-050 also adds a third quarantine object, `quarantine/<sha256>.extracted.md`.
>
> **(Amendment 2026-09-24, RFC-050 Iter 8): the supersession above is REVERSED.** RFC-050 D6 (unbounded retention) was struck because it contradicts Hard Rule 5 ("expiring within 30 days"). The 30-day `quarantine/` lifecycle TTL is restored, now covering the `.extracted.*` objects too, and is implemented as [[RFC-050]] Task 3.6. Every TTL clause in this RFC applies again. Task 7.5c is reopened and delivered through RFC-050 Task 3.6.
...
5. **(Amendment 2026-09-23, iter 2)** WHEN a document's bytes persist successfully (`save_doc` in `_persist_tree_result`, `save_flat_doc` in `_persist_flat_result`), THE indexer SHALL idempotently delete `quarantine/<sha256>.json` and `.meta.json` for that `sha256`. ~~Independently, a MinIO lifecycle rule SHALL expire objects under `quarantine/` after 30 days.~~ **Superseded by [[RFC-050]] D6 (2026-09-24):** no lifecycle TTL; clear-on-success and on-demand erasure only.
