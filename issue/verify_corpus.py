...
CATS = {
    ...
    "Pferde-Kranken": "B",
    "Pferde-OP": "B",
    "Meutenversicherung": "B",
    "Hundehalterhaftpflicht": "B",
    "Pferdehalterhaftpflicht": "B",
    "Haftpflicht-Allgemeine": "C",
    "Hundeleben-Allgemeine": "C",
    "Kundeninformation": "C",
    "Tier-OP-Kranken-Allgemeine": "C",
}
        ...
        try:
            ...
            rows.append(
                (
                    ...
                    ok,
                    reason or "PASS",
                    _tree_node_count(struct),
                    _tree_depth(struct),
                    C._max_heading_level(md),
                    time.monotonic() - t0,
                )
            )
        except Exception as e:
            rows.append(
                (cat_of(f), f, False, f"ERR:{type(e).__name__}:{e}", 0, 0, 0, time.monotonic() - t0)
            )
    rows.sort(key=lambda r: (r[0], r[1]))
    print(
        f"{'cat':3} {'ok':3} {'reason':14} {'nodes':>6} {'depth':>6} {'maxH':>5} {'sec':>6}  file"
    )
    for cat, f, ok, reason, nc, dp, mh, sec in rows:
        print(
            f"{cat:3} {'Y' if ok else 'N':3} {reason[:14]:14} {nc:6} {dp:6} {mh:5} {sec:6.1f}  {f}"
        )
