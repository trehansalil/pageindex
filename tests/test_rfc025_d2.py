...
1. ``check_garble`` with FLAT_MARKDOWN context returns ``True`` for post-retry
   text under 200 chars whose original tree-build failure reason was
   ``"garbling"`` or ``"node_garbling"`` -- BEFORE falling through to the
   size-floor heuristics that previously let short garbled text bypass the
   gate. Any other reason (or no reason) still gets normal evaluation, and
...
from pageindex_mcp.helpers import GarbleContext, TreeDefect, check_garble
...
class TestGarbleByDefaultShortPostRetryText:
    def test_a_short_text_with_garbling_reason_is_garbled(self, monkeypatch):
        ...
        assert check_garble(_SHORT_CLEAN_TEXT, expected_script=None, context=GarbleContext.FLAT_MARKDOWN, original_defect=TreeDefect.GARBLING) is True
    ...
    def test_b_short_text_with_node_garbling_reason_is_garbled(self, monkeypatch):
        ...
        monkeypatch.setattr(helpers, "_GARBLE_SHORT_TEXT_DEFAULT", True)
        assert check_garble(_SHORT_CLEAN_TEXT, expected_script=None, context=GarbleContext.FLAT_MARKDOWN, original_defect=TreeDefect.NODE_GARBLING) is True

    def test_c_short_text_with_unrelated_reason_gets_normal_evaluation(self, monkeypatch):
        monkeypatch.setattr(helpers, "_GARBLE_SHORT_TEXT_DEFAULT", True)
        assert check_garble(_SHORT_CLEAN_TEXT, expected_script=None, context=GarbleContext.FLAT_MARKDOWN, original_defect=TreeDefect.NODE_COUNT_LOW) is False
    ...
    def test_d_rollback_env_restores_prior_behavior(self, monkeypatch):
        ...
        assert check_garble(_SHORT_CLEAN_TEXT, expected_script=None, context=GarbleContext.FLAT_MARKDOWN, original_defect=TreeDefect.GARBLING) is False
