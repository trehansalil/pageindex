...
_GATE_CASES = [
    (TreeDefect.GARBLING, [{"title": "R", "text": "", "nodes": [_leaf(f"G{i}", _PUA) for i in range(3)]}], {}),
    (TreeDefect.NODE_COUNT_LOW, [_leaf("Root", _varied(1))], {}),
    (TreeDefect.DEPTH_LOW, [_leaf("A", _varied(1)), _leaf("B", _varied(2)), _leaf("C", _varied(3))], {}),
    (TreeDefect.NODE_GARBLING, [{"title": "R", "text": "", "nodes": [_leaf(f"G{i}", _PUA) for i in range(3)] + [_leaf(f"C{i}", _varied(i)) for i in range(3)]}], {}),
    (TreeDefect.REORDERED, [{"title": "R", "text": "", "nodes": [_leaf("A", _varied(1), start_index=10), _leaf("B", _varied(2), start_index=30), _leaf("C", _varied(3), start_index=20)]}], {}),
    (TreeDefect.RTL_REVERSAL, [{"title": "R", "text": "", "nodes": [_leaf(f"A{i}", _arabic(i)[::-1]) for i in range(3)]}], {}),
    (TreeDefect.BIDI_DEGRADED, [{"title": "R", "text": "", "nodes": [_leaf(f"A{i}", _arabic(i)[::-1]) for i in range(3)]}], {}),
    (TreeDefect.EMPTY_NODE_CONTAMINATION, [{"title": "R", "text": "", "nodes": [_leaf("", "") for _ in range(5)] + [_leaf("X", _varied(9))]}], {}),
    (TreeDefect.LOW_CONTENT_DENSITY, [{"title": "R", "text": "", "nodes": [_leaf(f"sec{i}", f"body{i}") for i in range(250)]}], {}),
    (TreeDefect.SUSPECT_DENSITY, [{"title": "R", "text": "", "nodes": [_leaf(f"S{i}", _varied(i, 5)) for i in range(3)]}], {"page_count": 500}),
]
...
@pytest.mark.parametrize(("defect", "tree", "kwargs"), _GATE_CASES,
...
class TestCoFiring:
    ...
    def test_rtl_and_bidi_co_fire(self):
        tree = [{"title": "R", "text": "", "nodes": [_leaf(f"A{i}", _arabic(i)[::-1]) for i in range(3)]}]
        result = validate_tree(tree)
