...
The memory-admission gate (``pageindex_mcp.worker.job.wait_for_memory``) reads the
...
``patch("pageindex_mcp.worker.job.wait_for_memory", ...)``; that inner patch takes
...
def _instant_memory_gate():
    ...
    try:
        with patch("pageindex_mcp.worker.job.wait_for_memory", _proceed):
            yield
