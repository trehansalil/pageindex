...
| 6 | MOU MOHRE & Nafis & وزارة الصناعة والتكنولوجيا المتقدمة (1).pdf | unknown | ERROR | **Re-test (RFC-045 fixes):** still rejected as `low_quality_tree: garbling` — latin_gibberish prong fires (not the PF false-positive). Defects: depth<2, suspect_density, latin_gibberish, figure marker mismatch, orphan fence markers. VLM fallback (gpt-4.1) attempted but did not rescue. This is a genuine garble issue unrelated to the NFKC PF fallback bug. No `processed/` artifacts in MinIO. |
...
- **MOU MOHRE & Nafis & وزارة الصناعة والتكنولوجيا المتقدمة (1).pdf** — MARGINAL -> ERROR. Run 7 (D0 OCR recovery) reached MARGINAL: tree, 20 nodes, depth 5, 14.6k chars, leaf_concentration=0.50. Run 8 is rejected outright by the quality gate for script mismatch (Arabic/Latin), shallow depth, suspect density, figure-marker mismatches, and orphan fence markers.
  *Hypothesis:* A prior OCR/garble recovery (D0) that got this bilingual doc to MARGINAL in Run 7 appears to have regressed or was not carried forward; the multi-signal garble rejection (script mismatch + fence markers + figure mismatches) suggests the same OCR/enrichment path that fixed it in Run 7 is no longer engaging for this file.
...
| 6 | MOU MOHRE & Nafis | MARGINAL -> ERROR | rejected (latin_gibberish + fence markers) | Garble-gate rejection (genuine) | NOT the PF false-positive — latin_gibberish prong fires on actual garbled content; VLM fallback attempted, did not rescue |
...
|-----|------|-------|----------|-----------------|
| 6 | MOU MOHRE & Nafis | ERROR (garbling) | **ERROR** (garbling) | Unchanged — latin_gibberish prong (genuine garble), not PF false-positive. VLM fallback attempted, did not rescue. |
