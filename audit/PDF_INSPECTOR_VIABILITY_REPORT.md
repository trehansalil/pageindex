...
+    "pdf_classification": pdf_classification.classification if has_pdf_inspector else None,
+    "pdf_confidence": pdf_classification.confidence if has_pdf_inspector else None,
+    "pages_needing_ocr": pdf_classification.pages_needing_ocr if has_pdf_inspector else [],
+    "has_encoding_issues": pdf_classification.has_encoding_issues if has_pdf_inspector else False,
 }
```
...
```json
{
  ...
  "pdf_classification": "scanned_with_text",
  "pdf_confidence": 0.94,
...
ocr_pages = handshake.get("pages_needing_ocr", [])
if pdf_class and ocr_pages:
    logger.info(f"PDF classifier: {pdf_class} (confidence {handshake.get('pdf_confidence'):.2f}), "
                f"OCR needed on pages {ocr_pages}")
```
...
- [ ] Integrate pdf-inspector's `detect_pdf_bytes()` into `probe_conversion_route()` with ~50ms timeout.
- [ ] Emit classification and confidence to `arq` job metadata and Prometheus for post-hoc analysis.
- [ ] Implement text-based routing: pass `DOCLING_DO_OCR=0` if confidence ≥ 0.95.
- [ ] Implement scanned routing: pass `DOCLING_DO_OCR=1` and capture per-page list for future mixed-doc support.
