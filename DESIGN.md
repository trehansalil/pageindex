...
- **LLM endpoint is fully configurable.** Both the query plane (OpenAI SDK) and ingestion
  ...
  set via `OPENAI_BASE_URL` + `LLM_PROVIDER` env vars. Supports OpenAI, Azure, OpenAI-compatible
  endpoints (vLLM, Together, Groq, OpenRouter, local), and self-hosted models — enabling zero
  data egress or controlled residency per deployment jurisdiction.
